...
async function renderCapacitacao() {
  ...
  if (!document.getElementById('cap-styles')) {
    ...
    style.textContent = `
      ...
      .cap-mod-cover { width:100%;aspect-ratio:4/3;position:relative;overflow:hidden;background:linear-gradient(135deg,#3D0E20,#6B1A3A);display:flex;align-items:flex-end;justify-content:flex-start; }
  ...
  modulos.forEach((mod, mi) => {
    ...
    html += `
      <div class="cap-mod-card" onclick="_abrirModuloModal('${mod.id}')" style="background:#fff;border:.5px solid #E8D9C5;border-radius:14px;overflow:hidden;cursor:pointer;transition:transform .15s ease,box-shadow .15s ease;width:155px;flex-shrink:0;scroll-snap-align:start;">
