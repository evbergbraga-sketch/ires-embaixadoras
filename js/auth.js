...
// ── Sanitização XSS ──────────────────────────
// Converte qualquer string em texto seguro para
// ser inserido via innerHTML. Impede XSS stored.
function s(str) {
  if (str == null) return '';
...
async function renderTopbar(opts = {}) {
  ...
  topbarRight.innerHTML = `
    ...
    " onclick="window.location.href='painel.html#perfil'" title="${s(profile.full_name)}">
      ${profile.avatar_url
        ? `<img src="${s(profile.avatar_url)}" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>`
        : `<span style="font-size:12px;font-weight:700;color:rgba(240,220,192,.9)">${s(initials(profile.full_name))}</span>`
      }
    </div>
    ...
    ${(data||[]).length ? (data||[]).map(a => `
      <div style="padding:11px 16px;border-bottom:0.5px solid #f0e5dc;cursor:pointer"
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
          ${tipoIcone(a.type)}
          <span style="font-size:12px;font-weight:700;color:#1e0a12">${s(a.subject) || 'Aviso'}</span>
        </div>
        <div style="font-size:11px;color:#7a6860;line-height:1.5;font-weight:500">${s((a.body||'').slice(0,90))}${(a.body||'').length>90?'…':''}</div>
